...
immed_re  = re.compile(r'''^(?P<instr>(ori|addi|andi|slti))\s+(?P<rt>\$r[0,1,2,3])\s+(?P<rs>\$r[0,1,2,3])\s+(?P<immed>-?(0x)?[0-9a-fA-F]+)$''')
...
simd_re   = re.compile(r'''^(?P<instr>(_simd_or|_simd_and|_simd_add|_simd_sub|_simd_sllv|_simd_srlv|_simd_srav|_simd_slt))\s+(?P<rd>\$r[0,1,2,3])\s+(?P<rs>\$r[0,1,2,3])\s+(?P<rt>\$r[0,1,2,3])$''')
...
def assemble_instructions(inputFile):
  ...
  for line in inputFile:
    ...
    simd     = simd_re.match(instruction)
    ...
    if len(instruction) != 0:
      ...
      if rtype:
        ...
        rs = int(simd.group('rs')[2])
        rt = int(simd.group('rt')[2])
        rd = int(simd.group('rd')[2])
        funct = functs[simd.group('instr')]
        num = 0xE << 12 | rs << 10 | rt << 8 | rd << 6 | funct
        debug("instruction: %s simd: rs: %d rt: %d rd: %d funct:%d num: %04x" % (instruction,rs,rt,rd,funct,num))
      else:
